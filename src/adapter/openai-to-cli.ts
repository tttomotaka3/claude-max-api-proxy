...
function toolsToPromptBlock(tools: OpenAITool[]): string {
  const lines: string[] = [
    ...
    "IMPORTANT: The JSON inside <tool_use> must be valid JSON on a single line. Do NOT use newlines or indentation inside the JSON.",
    "IMPORTANT: Do NOT wrap the JSON in markdown code fences (```). Output the raw JSON directly between the tags.",
    "IMPORTANT: The JSON must contain exactly two keys: \"name\" (string) and \"arguments\" (object). No other format is accepted.",
    "",
    "Available tools:",
    "",
  ];
